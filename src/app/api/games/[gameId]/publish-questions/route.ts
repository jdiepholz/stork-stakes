...
export async function POST(req: NextRequest, { params }: { params: Promise<{ gameId: string }> }) {
  ...
  try {
    ...
    if (game.createdBy !== userId) {
      return NextResponse.json({ error: 'Only the game creator can publish questions' }, { status: 403 });
    }
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ gameId: string }> }) {
  ...
  try {
    ...
    if (game.createdBy !== userId) {
      return NextResponse.json({ error: 'Only the game creator can unpublish questions' }, { status: 403 });
    }
